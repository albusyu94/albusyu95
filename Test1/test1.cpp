...
static void drawQRCodeContour(Mat &color_image, vector<Point> transform);  //画轮廓
static void drawFPS(Mat &color_image, double fps); //帧率	
static int  liveQRCodeDetect(const string& out_file); //QR提取
static int  imageQRCodeDetect(const string& in_file, const string& out_file); //图片模式
